...
if [[ -z "$source_date_epoch" ]]; then
  if ! git -C "$repo_root" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    printf 'SOURCE_DATE_EPOCH is required outside a Git working tree.\n' >&2
    exit 1
  fi
...
if [[ ! "$source_date_epoch" =~ ^[0-9]+$ ]]; then
  printf 'SOURCE_DATE_EPOCH must be a Unix timestamp: %s\n' "$source_date_epoch" >&2
  exit 2
fi
...
SOURCE_DATE_EPOCH="$source_date_epoch" APPIMAGE_EXTRACT_AND_RUN=1 ARCH=x86_64 "$appimagetool" --runtime-file "$runtime" "$appdir" "$output"
