    ...
    public class NormalTimeToken : TimeToken
    {
        ...
        public override string ToString(IFormatProvider provider)
        {
            try
            {
                ...
                // Hour
                int adjustedHour;
                if (ParserSettings.Prefer24HourTime)
                {
                ...
                // Minute
                if (this.Minute != 0 || this.Second != 0 || this.HourPeriod == HourPeriod.Undefined || ParserSettings.Prefer24HourTime)
                {
                    stringBuilder.AppendFormat(
                        Resources.ResourceManager.GetEffectiveProvider(provider),
        ...
        public new class Parser : TimeToken.Parser
        {
            ...
            protected override TimeToken ParseInternal(Match match, IFormatProvider provider)
            {
                ...
                else if (match.Groups["military"].Success || ParserSettings.Prefer24HourTime)
                {
                    if (timeToken.Hour == 0)
                    {
                        timeToken.Hour = 12;
